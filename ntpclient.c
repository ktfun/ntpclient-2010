...
static int ntp_client_is_alive = 0;  /* ntp client是否存活的标志 */
static int ntp_error = -1;
...
static int setup_transmit(int usd, char *host, short port, struct ntp_control *ntpc)
{
	...
	if (ret != 0)
	{
		printf("error: stuff_net_addr ret = %d func: %s line: %d\n", ret, __FUNCTION__, __LINE__);
		return -1;
	}
	...
	{
		printf("error: connect func: %s line: %d\n", __FUNCTION__, __LINE__);
		return -1;
	}
...
#define sizeof_incoming (sizeof incoming_word)
	...
	for (;;) 
	{
		...
		if ((i!=1)||(!FD_ISSET(usd,&fds))) 
		{
			...
			if (to.tv_sec == 0 && time_count != ntpc->cycle_time && !is_first_time) 
			{
				time_count++;
			}
			else
...
void* ntp_client_start(int cycle_time, int debug_on, char *hostname, int cross_check, int set_clock, int live)
{
	...
	ntp_client_is_alive = 1;

	if (cycle_time < 0 || hostname == NULL)
	{
		printf("cycle_time < 0 || hostname == NULL \n");
		return (void*)-1;
	}

	ntpc.live = live;
	ntpc.set_clock = set_clock;
	ntpc.probe_count = 0;           /* default of 0 means loop forever */
	ntpc.cycle_time = cycle_time;          /* seconds */
	ntpc.goodness = 0;
	ntpc.cross_check = cross_check;
	...
	ret = setup_receive(usd, INADDR_ANY, udp_local_port);
	if (ret != 0)
	{
		printf ("error: setup_receive ret = %d func: %s line: %d\n", ret, __FUNCTION__, __LINE__);
		return (void*)-1;
	}
	...
	if (ret != 0)
	{
		printf ("error: setup_transmit ret = %d func: %s line: %d\n", ret, __FUNCTION__, __LINE__);
		return (void*)-1;
	}
